...
IF NOT EXISTS (SELECT * FROM AiKnowledgeItems WHERE Title = 'Release v2026.08.21 Summary')
BEGIN
...
IF NOT EXISTS (SELECT * FROM AiKnowledgeItems WHERE Title = 'How to Use AI Copilot Assistant')
BEGIN
    INSERT INTO AiKnowledgeItems (Title, Category, Tags, Content, Source, IsActive, CreatedAt, UpdatedAt)
...
IF NOT EXISTS (SELECT * FROM AiKnowledgeItems WHERE Title = 'Rule 4: Separate Money Flows Governance')
BEGIN
    INSERT INTO AiKnowledgeItems (Title, Category, Tags, Content, Source, IsActive, CreatedAt, UpdatedAt)
...
IF NOT EXISTS (SELECT * FROM AiKnowledgeItems WHERE Title = 'How to Configure User Accounts & Permissions')
BEGIN
    INSERT INTO AiKnowledgeItems (Title, Category, Tags, Content, Source, IsActive, CreatedAt, UpdatedAt)
...
IF NOT EXISTS (SELECT * FROM AiKnowledgeItems WHERE Title = 'Automated Tour Status Checkpoints (Gatekeeping)')
BEGIN
    INSERT INTO AiKnowledgeItems (Title, Category, Tags, Content, Source, IsActive, CreatedAt, UpdatedAt)
